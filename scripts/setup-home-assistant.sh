...
Options:
  ...
  --binary PATH     Geist binary (auto-detected if omitted)
...
if [ -z "$binary" ]; then
    for candidate in \
        "$root/geist" \
        "$HOME/.local/bin/geist" \
        /usr/local/bin/geist; do
        if [ -x "$candidate" ]; then binary=$candidate; break; fi
    done
fi
if [ -z "$binary" ] || [ ! -x "$binary" ]; then
    echo "setup-home-assistant: Geist binary not found for $release_platform" >&2
    echo "Download it from a Geist release or pass --binary /absolute/path/geist." >&2
    exit 1
fi
